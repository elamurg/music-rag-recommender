...
# client objects
# pylast network is the read/write client — read-only calls don't sign requests
...
# searches across all artists, the closest equivalent to spotify's free-text search.
# get_next_page() returns the first page of results as Track objects
...
# across all music services, ideal as a primary key in your corpus
...
# the raw text often contains html tags like <a href="...">; strip in production
...
if genius_song and genius_song.lyrics:
    # the returned lyrics string often starts with metadata noise like
    # "5 ContributorsBlinding Lights Lyrics" — in production you'd clean
    # this with a regex; for a sanity check we just truncate
